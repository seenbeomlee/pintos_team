...
thread_test_preemption (void)
{
  ...
  list_entry (list_max (&ready_list, thread_compare_priority, NULL), struct thread, elem)->priority)
    thread_yield();
}
...
refresh_priority (void)
{
  ...
  if (!list_empty (&cur->donations)) {
    ...
    struct thread *max = list_entry (list_max (&cur->donations, thread_compare_donate_priority, NULL), struct thread, donation_elem);
